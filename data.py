...
class BagDescription:
    ...
    def from_bag(cls, map_data: Map, bag: Bag):
        ...
        return cls(weight=sum(g.weight for g in gifts),
                   volume=sum(g.volume for g in gifts))
...
class Line:
    ...
    def distance_in_circle(self, circle: 'Circle') -> float:
        ...
        distance_from_center_sq = (self.a * circle.center.x + self.b * circle.center.y +
        ...
        if self.from_pos.in_circle(circle):
            if self.to_pos.in_circle(circle):
                ...
                return sqrt(r_sq - distance_from_center_sq) + \
                       sqrt(circle.center.dist(self.from_pos) ** 2 - distance_from_center_sq)
        elif self.to_pos.in_circle(circle):
            return sqrt(r_sq - distance_from_center_sq) + \
                   sqrt(circle.center.dist(self.to_pos) ** 2 - distance_from_center_sq)
